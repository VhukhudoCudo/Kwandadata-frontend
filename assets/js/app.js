...
import { initProofOfAction } from './proof-of-action.js';
 import Chart from 'chart.js/auto';
 window.Chart = Chart;

const PAGES = {};

PAGES['splash'] = `<div class="splash-screen"><div class="splash-bubble splash-bubble-1"></div><div class="splash-bubble splash-bubble-2"></div><div class="splash-bubble splash-bubble-3"></div><div class="splash-center"><img src="assets/img/logo.png" alt="KwandaData" class="splash-logo-img"/><h1 class="splash-title">KwandaData&trade;</h1><p class="splash-tagline">Connect. Earn. Empower Communities.</p><p class="splash-sub">Turning Participation into Opportunity</p></div><div class="splash-actions"><button onclick="navigateTo('register-type')" style="width:100%;padding:16px;border-radius:30px;background:#fff;color:#2d1b8e;font-size:15px;font-weight:700;border:none;cursor:pointer;font-family:sans-serif;margin-bottom:12px;">Register</button><button onclick="navigateTo('signin-type')" style="width:100%;padding:16px;border-radius:30px;background:transparent;color:#fff;font-size:15px;font-weight:600;border:1.5px solid rgba(255,255,255,0.3);cursor:pointer;font-family:sans-serif;">Sign In</button></div><div class="splash-bar"></div></div>`;

PAGES['register-type'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="navigateTo('splash')"><i class="ti ti-arrow-left"></i></button><h1>Register</h1><p>Choose the type of account you want to create</p></div><div class="auth-body"><button onclick="navigateTo('create-account')" style="width:100%;padding:20px;border-radius:20px;background:#fff;border:1.5px solid var(--border);cursor:pointer;font-family:sans-serif;display:flex;align-items:center;gap:14px;text-align:left;"><span style="width:48px;height:48px;border-radius:14px;background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;">👤</span><span><span style="display:block;font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:2px;">User</span><span style="display:block;font-size:12px;color:var(--text-muted);">Complete activities and earn money</span></span></button><button onclick="navigateTo('advertiser-register')" style="width:100%;padding:20px;border-radius:20px;background:#fff;border:1.5px solid var(--border);cursor:pointer;font-family:sans-serif;display:flex;align-items:center;gap:14px;text-align:left;"><span style="width:48px;height:48px;border-radius:14px;background:#fff7ed;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;">🏢</span><span><span style="display:block;font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:2px;">Advertiser</span><span style="display:block;font-size:12px;color:var(--text-muted);">Run campaigns and reach users</span></span></button><p class="auth-footer">Already have an account? <span onclick="navigateTo('sign-in')">Sign In</span></p></div></div>`;
PAGES['sign-in'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="navigateTo('splash')"><i class="ti ti-arrow-left"></i></button><h1>Welcome Back</h1><p>Sign in to your KwandaData account</p></div><div class="auth-body"><div class="form-group"><label for="login-email">Email Address</label><div class="input-wrap has-icon-left"><i class="ti ti-mail"></i><input type="email" id="login-email" placeholder="john@example.com" autocapitalize="off" autocorrect="off" spellcheck="false"/></div></div><div class="form-group"><label for="login-password">Password</label><div class="input-wrap has-icon-both"><i class="ti ti-lock"></i><input type="password" id="login-password" placeholder="Enter your password"/><i class="ti ti-eye icon-right" onclick="togglePassword('login-password', this)"></i></div></div><p class="forgot-link" onclick="navigateTo('forgot-password')">Forgot Password?</p><p class="auth-error" id="login-error"></p><button class="btn-purple" onclick="handleLogin()">Sign In</button><p class="auth-footer">Don't have an account? <span onclick="navigateTo('create-account')">Create Account</span></p></div></div>`;
...
PAGES['proof-of-action'] = `<div class="wallet-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('home')"><i class="ti ti-arrow-left"></i></button><h2>Proof of Campaign Action</h2></div><button class="icon-btn"><i class="ti ti-info-circle"></i></button></div><div class="page-scroll"><div class="wallet-card"><div><p class="wallet-label">Proof of Campaign Action</p><p class="wallet-amount" id="poa-total-pending">R 0.00</p><p class="wallet-sub">Pending until you confirm a purchase</p></div><div class="wallet-icon"><i class="ti ti-clock"></i></div></div><div style="background:#fff7ed;border:1.5px solid #f97316;border-radius:14px;padding:14px 16px;margin:0 16px 16px;"><p style="font-size:12px;color:#9a3412;margin:0;line-height:1.5;">Watch a campaign video, then visit that brand's shop and upload a photo of your receipt to unlock this money into your Hello Wallet. Unredeemed amounts expire after 30 days.</p></div><div style="background:#fff;border-radius:14px;padding:14px 16px;border:1px solid var(--border);margin:0 16px 16px;display:flex;align-items:center;gap:10px;"><i class="ti ti-users" style="font-size:20px;color:var(--primary);flex-shrink:0;"></i><p id="poa-referral-stats" style="font-size:13px;color:var(--text-primary);margin:0;line-height:1.5;">Loading referral earnings...</p></div><div class="section"><div class="section-header"><p class="section-title">Pending</p></div><div id="poa-entries-list" style="padding:0 16px;"></div></div><div class="section"><div class="section-header"><p class="section-title">History</p></div><div id="poa-history-list" style="background:#fff;border-radius:14px;border:1px solid var(--border);padding:4px 16px;margin:0 16px;"></div></div></div><nav class="bottom-nav"><button class="nav-item" onclick="navigateTo('home')"><i class="ti ti-home"></i><span>Home</span></button><button class="nav-item" onclick="navigateTo('earn')"><i class="ti ti-database"></i><span>Earn</span></button><button class="nav-item" onclick="navigateTo('wallet')"><i class="ti ti-wallet"></i><span>Wallet</span></button><button class="nav-item" onclick="navigateTo('redeem')"><i class="ti ti-refresh"></i><span>Redeem</span></button><button class="nav-item" onclick="navigateTo('donate')"><i class="ti ti-heart"></i><span>Donate</span></button><button class="nav-item" onclick="navigateTo('profile')"><i class="ti ti-user"></i><span>Profile</span></button></nav></div>`;
...
PAGES['wallet'] = `<div class="wallet-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('home')"><i class="ti ti-arrow-left"></i></button><h2>My Wallet</h2></div><button class="icon-btn"><i class="ti ti-world"></i></button></div><div class="page-scroll"><div class="wallet-card"><div><p class="wallet-label">Available Balance</p><p class="wallet-amount">R 0.00</p><p class="wallet-sub">KwandaData Wallet</p></div><div class="wallet-icon"><i class="ti ti-wallet"></i></div></div><div class="bonus-card" onclick="navigateTo('proof-of-action')" style="cursor:pointer;"><div><p class="bonus-label">Proof of Campaign Action</p><p class="bonus-amount">R 0.00</p></div><div class="bonus-icon"><i class="ti ti-gift"></i></div></div><div style="background:linear-gradient(135deg,#1d4ed8,#3b82f6);border-radius:14px;padding:14px 18px;margin:10px 16px 0;display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:12px;color:rgba(255,255,255,0.8);margin-bottom:2px;">📱 Ready-to-Use Airtime</p><p class="data-balance" style="font-size:22px;font-weight:700;color:#fff;">R 0.00</p><p style="font-size:11px;color:rgba(255,255,255,0.7);">Ready to redeem as airtime</p></div><div style="width:44px;height:44px;background:rgba(255,255,255,0.2);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;color:#fff;"><i class="ti ti-device-mobile"></i></div></div><div class="wallet-card" onclick="navigateTo('campaign-wallet')" style="cursor:pointer;"><div><p class="wallet-label">Campaign Objective Wallet</p><p class="wallet-amount" id="campaign-wallet-total">R 0.00</p><p class="wallet-sub">Redeemable with specific companies</p></div><div class="wallet-icon"><i class="ti ti-building-store"></i></div></div><div class="wallet-card" onclick="navigateTo('personal-goals')" style="cursor:pointer;"><div><p class="wallet-label">Personal Goals Wallet</p><p class="wallet-amount" id="goals-wallet-total">R 0.00</p><p class="wallet-sub">Saved toward your personal goals</p></div><div class="wallet-icon"><i class="ti ti-target-arrow"></i></div></div><div class="section"><div class="section-header"><p class="section-title">Transaction History</p><select class="tx-filter" onchange="filterTransactions(this.value)"><option value="all">All</option><option value="earned">Earned</option><option value="redeemed">Redeemed</option></select></div><div class="tx-list" id="tx-list"></div></div></div><nav class="bottom-nav"><button class="nav-item" onclick="navigateTo('home')"><i class="ti ti-home"></i><span>Home</span></button><button class="nav-item" onclick="navigateTo('earn')"><i class="ti ti-database"></i><span>Earn</span></button><button class="nav-item active"><i class="ti ti-wallet"></i><span>Wallet</span></button><button class="nav-item" onclick="navigateTo('redeem')"><i class="ti ti-refresh"></i><span>Redeem</span></button><button class="nav-item" onclick="navigateTo('donate')"><i class="ti ti-heart"></i><span>Donate</span></button><button class="nav-item" onclick="navigateTo('profile')"><i class="ti ti-user"></i><span>Profile</span></button></nav></div>`;

PAGES['buy-data-info'] = `<div class="wallet-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('redeem')"><i class="ti ti-arrow-left"></i></button><h2>Buy Data</h2></div></div><div class="page-scroll"><div style="background:linear-gradient(135deg,#1d4ed8,#3b82f6);border-radius:14px;padding:16px 18px;margin:16px 16px 0;"><p style="font-size:13px;color:#fff;line-height:1.6;">Redeem your earnings as airtime, then use that airtime to buy a data bundle directly from your network using the codes below.</p></div><div class="section"><div class="section-header"><p class="section-title">Dial these codes on your phone</p></div></div><div style="padding:0 16px;display:flex;flex-direction:column;gap:10px;"><div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin:0;">MTN</p><p style="font-size:12px;color:var(--text-muted);margin:2px 0 0;">Buy a data bundle</p></div><p style="font-size:16px;font-weight:700;color:var(--primary);margin:0;">*136*2#</p></div><div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin:0;">Vodacom</p><p style="font-size:12px;color:var(--text-muted);margin:2px 0 0;">Buy a data bundle</p></div><p style="font-size:16px;font-weight:700;color:var(--primary);margin:0;">*135#</p></div><div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin:0;">Cell C</p><p style="font-size:12px;color:var(--text-muted);margin:2px 0 0;">Buy a data bundle</p></div><p style="font-size:16px;font-weight:700;color:var(--primary);margin:0;">*147#</p></div><div style="background:#fff;border-radius:12px;padding:14px 16px;border:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin:0;">Telkom</p><p style="font-size:12px;color:var(--text-muted);margin:2px 0 0;">Buy a data bundle</p></div><p style="font-size:16px;font-weight:700;color:var(--primary);margin:0;">*180#</p></div></div><p style="font-size:11px;color:var(--text-muted);padding:12px 16px 24px;line-height:1.5;">Codes can change from time to time and may vary by SIM or promotion — if a code doesn't work, check your network's official app or website for the latest option.</p></div></div>`;

PAGES['redeem'] = `<div class="redeem-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('home')"><i class="ti ti-arrow-left"></i></button><h2>Redeem / Use</h2></div><button class="icon-btn"><i class="ti ti-info-circle"></i></button></div><div class="page-scroll"><div class="wallet-card"><div><p class="wallet-label">Your Balance</p><p class="wallet-amount">R 0.00</p></div><div class="wallet-icon"><i class="ti ti-cloud-upload"></i></div></div><div class="section"><div class="section-header"><p class="section-title">Redeem Data</p></div></div><div class="redeem-option" onclick="handleRedeem('airtime')"><div class="redeem-icon violet"><i class="ti ti-device-mobile"></i></div><div class="redeem-info"><h4>Airtime</h4><p>Buy airtime with your earnings</p></div><i class="ti ti-chevron-right redeem-arrow"></i></div><div class="redeem-option" onclick="navigateTo('buy-data-info')"><div class="redeem-icon blue"><i class="ti ti-wifi"></i></div><div class="redeem-info"><h4>Data Bundle</h4><p>Buy data using your airtime</p></div><i class="ti ti-chevron-right redeem-arrow"></i></div><div class="redeem-option" onclick="handleRedeem('partner-apps')"><div class="redeem-icon green"><i class="ti ti-apps"></i></div><div class="redeem-info"><h4>Partner Apps</h4><p>Spend on partner platforms</p></div><i class="ti ti-chevron-right redeem-arrow"></i></div><div class="redeem-option" onclick="handleRedeem('cash-out')"><div class="redeem-icon red"><i class="ti ti-banknote"></i></div><div class="redeem-info"><h4>Cash Out</h4><p>Withdraw to your bank account</p></div><i class="ti ti-chevron-right redeem-arrow"></i></div><div class="section"><div class="section-header"><p class="section-title">📱 Ready-to-Use Airtime</p></div></div><div style="background:linear-gradient(135deg,#1d4ed8,#3b82f6);border-radius:14px;padding:16px 18px;margin:0 16px 16px;display:flex;justify-content:space-between;align-items:center;"><div><p style="font-size:12px;color:rgba(255,255,255,0.8);margin-bottom:2px;">Ready-to-Use Airtime</p><p class="data-balance-redeem" style="font-size:24px;font-weight:700;color:#fff;">R 0.00</p><p style="font-size:11px;color:rgba(255,255,255,0.7);">Redeem as airtime to any number</p></div><i class="ti ti-device-mobile" style="font-size:32px;color:rgba(255,255,255,0.6);"></i></div><div style="padding:0 16px 16px;"><button onclick="redeemAirtimeFromWallet()" style="width:100%;padding:13px;border-radius:30px;background:linear-gradient(135deg,#1d4ed8,#3b82f6);color:#fff;font-size:14px;font-weight:700;border:none;cursor:pointer;">Redeem Airtime</button></div><div class="section"><div class="section-header"><p class="section-title">Recent Redemptions</p></div><div class="redemption-list" id="redemption-list"></div></div></div><nav class="bottom-nav"><button class="nav-item" onclick="navigateTo('home')"><i class="ti ti-home"></i><span>Home</span></button><button class="nav-item" onclick="navigateTo('earn')"><i class="ti ti-database"></i><span>Earn</span></button><button class="nav-item" onclick="navigateTo('wallet')"><i class="ti ti-wallet"></i><span>Wallet</span></button><button class="nav-item active"><i class="ti ti-refresh"></i><span>Redeem</span></button><button class="nav-item" onclick="navigateTo('donate')"><i class="ti ti-heart"></i><span>Donate</span></button><button class="nav-item" onclick="navigateTo('profile')"><i class="ti ti-user"></i><span>Profile</span></button></nav></div>`;
...
PAGES['verify-email'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><h1>Email Verification</h1><p>Confirming your KwandaData account</p></div><div class="auth-body" style="text-align:center;"><div id="verify-email-icon" style="font-size:52px;margin:20px 0;color:var(--primary);"><i class="ti ti-loader-2"></i></div><p id="verify-email-message" style="font-size:14px;color:var(--text-muted);margin-bottom:20px;">Verifying your email...</p><button class="btn-purple" onclick="navigateTo('home')" id="verify-email-continue-btn" style="display:none;">Continue to KwandaData</button></div></div>`;

//PAGES['forgot-password'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="navigateTo('sign-in')"><i class="ti ti-arrow-left"></i></button><h1>Forgot Password</h1><p>Reset your KwandaData password</p></div><div class="auth-body"><div style="background:linear-gradient(135deg,#6c63ff,#2d1b8e);border-radius:18px;padding:20px;text-align:center;color:#fff;margin-bottom:16px;"><i class="ti ti-lock-open" style="font-size:40px;margin-bottom:10px;display:block;"></i><p style="font-size:13px;color:rgba(255,255,255,0.85);line-height:1.5;">Enter your registered email address and we will send you a link to reset your password.</p></div><div class="form-group"><label for="reset-email">Email Address</label><div class="input-wrap has-icon-left"><i class="ti ti-mail"></i><input type="email" id="reset-email" placeholder="john@example.com" autocapitalize="off" autocorrect="off" spellcheck="false"/></div></div><p class="auth-error" id="reset-error"></p><p id="reset-success" style="font-size:13px;color:var(--accent-green);text-align:center;min-height:16px;"></p><button class="btn-purple" onclick="handleForgotPassword()">Send Reset Link</button><p class="auth-footer"><span onclick="navigateTo('sign-in')">Back to Sign In</span></p></div></div>`;

PAGES['forgot-password'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="navigateTo('sign-in')"><i class="ti ti-arrow-left"></i></button><h1>Forgot Password</h1><p>Reset your KwandaData password</p></div><div class="auth-body"><div style="background:linear-gradient(135deg,#6c63ff,#2d1b8e);border-radius:18px;padding:20px;text-align:center;color:#fff;margin-bottom:16px;"><i class="ti ti-lock-open" style="font-size:40px;margin-bottom:10px;display:block;"></i><p style="font-size:13px;color:rgba(255,255,255,0.85);line-height:1.5;">Enter your registered email address and we will send you a link to reset your password.</p></div><div class="form-group"><label for="reset-email">Email Address</label><div class="input-wrap has-icon-left"><i class="ti ti-mail"></i><input type="email" id="reset-email" placeholder="john@example.com" autocapitalize="off" autocorrect="off" spellcheck="false"/></div></div><p class="auth-error" id="reset-error"></p><p id="reset-success" style="font-size:13px;color:var(--accent-green);text-align:center;min-height:16px;"></p><button class="btn-purple" onclick="handleForgotPassword()">Send Reset Link</button><p class="auth-footer"><span onclick="navigateTo('sign-in')">Back to Sign In</span></p></div></div>`;
PAGES['reset-password'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><h1>Set New Password</h1><p>Choose a new password for your account</p></div><div class="auth-body"><div class="form-group"><label for="new-reset-password">New Password</label><div class="input-wrap has-icon-both"><i class="ti ti-lock"></i><input type="password" id="new-reset-password" placeholder="Enter a new password"/><i class="ti ti-eye icon-right" onclick="togglePassword('new-reset-password', this)"></i></div></div><div class="form-group"><label for="confirm-reset-password">Confirm Password</label><div class="input-wrap has-icon-both"><i class="ti ti-lock"></i><input type="password" id="confirm-reset-password" placeholder="Repeat your new password"/><i class="ti ti-eye icon-right" onclick="togglePassword('confirm-reset-password', this)"></i></div></div><p class="auth-error" id="reset-password-error"></p><p id="reset-password-success" style="font-size:13px;color:var(--accent-green);text-align:center;min-height:16px;"></p><button class="btn-purple" onclick="handleResetPassword()">Set New Password</button><p class="auth-footer"><span onclick="navigateTo('sign-in')">Back to Sign In</span></p></div></div>`;

PAGES['terms'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="history.back()"><i class="ti ti-arrow-left"></i></button><h1>Terms of Service</h1><p>Last updated: August 2026</p></div><div class="auth-body"><div style="background:#fff;border-radius:14px;border:1px solid var(--border);padding:16px 18px;display:flex;flex-direction:column;gap:16px;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">1. Acceptance of Terms</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">By registering and using KwandaData, you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use our platform.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">2. Eligibility</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">You must be at least 18 years old and a resident of South Africa to use KwandaData.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">3. Earnings & Fees</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">By completing any task, survey, video, or sponsored campaign on KwandaData, you agree to the following earnings breakdown, calculated on the gross reward shown for that activity:</p><ul style="font-size:13px;color:var(--text-muted);line-height:1.6;margin:8px 0 0;padding-left:18px;"><li>A portion is deducted as an administration fee.</li><li>A portion is automatically converted to data (MB) and added to your Ready-to-Use Data Balance.</li><li>For sponsored campaign tasks, a further portion is added to a Campaign Objective Wallet tied to that campaign's advertiser, and for campaign video tasks specifically, an additional portion is held as a pending Proof of Campaign Action amount until a matching purchase is confirmed.</li><li>The remainder is added to your Wallet Balance in Rand.</li></ul><p style="font-size:13px;color:var(--text-muted);line-height:1.6;margin-top:8px;">Exact percentages may be adjusted by KwandaData from time to time and are not fixed by these Terms. This split applies automatically every time you complete an activity — you will not be asked to confirm it each time, as doing so constitutes acceptance of these Terms.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">4. Redemption Policy</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">Wallet balances can be redeemed for airtime, data bundles, partner app credits, or cash. Minimum redemption amounts apply.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">5. Referral Programme</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">Referral bonuses are credited to the referrer as soon as the referred user successfully registers using their referral code.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">6. Account Termination</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">KwandaData reserves the right to suspend or terminate accounts that violate these terms.</p></div></div></div></div>`;

PAGES['privacy'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="history.back()"><i class="ti ti-arrow-left"></i></button><h1>Privacy Policy</h1><p>Last updated: August 2026</p></div><div class="auth-body"><div style="background:#fff;border-radius:14px;border:1px solid var(--border);padding:16px 18px;display:flex;flex-direction:column;gap:16px;"><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">1. Information We Collect</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">We collect personal information including your name, email, phone number, date of birth, gender, home language, province, region, employment status, and, for advertiser accounts, your company and industry.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">2. How We Use Your Information</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">Your information is used to manage your account, process earnings and redemptions, and improve our services. We do not sell your personal information.</p></div><div><p style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">3. Your Rights (POPIA)</p><p style="font-size:13px;color:var(--text-muted);line-height:1.6;">You have the right to access, correct, or delete your personal information. Contact us at privacy@kwandadata.co.za.</p></div></div></div></div>`;
...
PAGES['admin-activities'] = `<div class="home-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('admin-panel')"><i class="ti ti-arrow-left"></i></button><h2>Activities</h2></div><button class="icon-btn" onclick="initAdminActivities()"><i class="ti ti-refresh"></i></button></div><div class="page-scroll"><div style="padding:16px 16px 8px;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;"><div style="background:#fff;border-radius:12px;padding:12px;border:1px solid var(--border);text-align:center;"><p id="act-total" style="font-size:22px;font-weight:700;color:var(--primary);">0</p><p style="font-size:11px;color:var(--text-muted);">Total Activities</p></div><div style="background:#fff;border-radius:12px;padding:12px;border:1px solid var(--border);text-align:center;"><p id="act-today" style="font-size:22px;font-weight:700;color:var(--accent-green);">0</p><p style="font-size:11px;color:var(--text-muted);">Last 24 Hours</p></div></div></div><div style="padding:0 16px;"><p style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:12px;">Recent Activities</p><div id="activities-list"><div style="text-align:center;padding:24px;color:var(--text-muted);font-size:13px;">Loading...</div></div></div></div></div>`;
...
PAGES['advertiser-notifications'] = `<div class="auth-screen"><div class="auth-header"><div class="auth-header-bubble auth-header-bubble-1"></div><div class="auth-header-bubble auth-header-bubble-2"></div><button class="auth-back-btn" onclick="navigateTo('advertiser-dashboard')"><i class="ti ti-arrow-left"></i></button><h1>Notifications</h1><p>Campaign updates and alerts</p></div><div class="auth-body"><div id="adv-notifications-list"><div style="text-align:center;padding:32px;color:var(--text-muted);"><i class="ti ti-bell-off" style="font-size:40px;display:block;margin-bottom:12px;opacity:0.4;"></i><p style="font-size:14px;font-weight:600;">No notifications yet</p><p style="font-size:12px;margin-top:4px;">Campaign updates will appear here</p></div></div></div><nav class="bottom-nav"><button class="nav-item" onclick="navigateTo('advertiser-dashboard')"><i class="ti ti-home"></i><span>Home</span></button><button class="nav-item" onclick="navigateTo('advertiser-campaigns')"><i class="ti ti-speakerphone"></i><span>Campaigns</span></button><button class="nav-item" onclick="navigateTo('advertiser-create-campaign')"><i class="ti ti-plus"></i><span>Create</span></button><button class="nav-item" onclick="navigateTo('advertiser-analytics')"><i class="ti ti-chart-bar"></i><span>Analytics</span></button><button class="nav-item" onclick="navigateTo('advertiser-profile')"><i class="ti ti-user"></i><span>Profile</span></button></nav></div></div>`;
...
// ══════════════════════════════════════
PAGES['admin-panel'] = `<div class="home-screen"><div class="home-header"><div><p class="home-greeting">Admin Panel</p><h2 class="home-name">KwandaData Admin</h2></div><button onclick="advertiserLogout()" style="background:#fee2e2;border:none;border-radius:20px;padding:8px 12px;display:flex;align-items:center;gap:5px;cursor:pointer;"><i class="ti ti-logout" style="color:#ef4444;font-size:15px;"></i><span style="color:#ef4444;font-size:12px;font-weight:600;">Log Out</span></button></div><div class="page-scroll"><div style="padding:16px 16px 0;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;"><div style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--border);text-align:center;"><p id="admin-panel-total" style="font-size:24px;font-weight:700;color:var(--primary);">0</p><p style="font-size:12px;color:var(--text-muted);">Total Campaigns</p></div><div style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--border);text-align:center;"><p id="admin-panel-pending" style="font-size:24px;font-weight:700;color:#f97316;">0</p><p style="font-size:12px;color:var(--text-muted);">Pending Redemptions</p></div><div style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--border);text-align:center;"><p id="admin-panel-active" style="font-size:24px;font-weight:700;color:#22c55e;">0</p><p style="font-size:12px;color:var(--text-muted);">Active Campaigns</p></div><div style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--border);text-align:center;"><p id="admin-panel-advertisers" style="font-size:24px;font-weight:700;color:#3b82f6;">0</p><p style="font-size:12px;color:var(--text-muted);">Total Advertisers</p></div></div></div><div class="section" style="padding:0 16px;"><p class="section-title" style="margin-bottom:12px;">Admin Controls</p><div class="quick-actions"><div class="action-card" onclick="navigateTo('admin-redemptions')"><div class="action-icon violet"><i class="ti ti-receipt"></i></div><h3>Redemptions</h3><p>Approve or reject payouts</p></div><div class="action-card" onclick="navigateTo('admin-activities')"><div class="action-icon green"><i class="ti ti-list-check"></i></div><h3>Activities</h3><p>Recent platform activity</p></div><div class="action-card" onclick="navigateTo('admin-users-mgmt')"><div class="action-icon blue"><i class="ti ti-users"></i></div><h3>User Management</h3><p>View, suspend, adjust users</p></div><div class="action-card" onclick="navigateTo('admin-campaigns-mgmt')"><div class="action-icon orange"><i class="ti ti-speakerphone"></i></div><h3>Campaigns</h3><p>Approve or reject campaigns</p></div><div class="action-card" onclick="navigateTo('admin-advertisers-mgmt')"><div class="action-icon green"><i class="ti ti-building-store"></i></div><h3>Advertisers</h3><p>Manage advertiser accounts</p></div><div class="action-card" onclick="navigateTo('admin-analytics')"><div class="action-icon purple"><i class="ti ti-chart-bar"></i></div><h3>Analytics</h3><p>Platform-wide stats</p></div><div class="action-card" onclick="navigateTo('admin-announcements')"><div class="action-icon blue"><i class="ti ti-speakerphone"></i></div><h3>Announcements</h3><p>Notify users and advertisers</p></div><div class="action-card" onclick="navigateTo('admin-settings')"><div class="action-icon orange"><i class="ti ti-settings"></i></div><h3>App Settings</h3><p>Pricing, fees and maintenance</p></div><div class="action-card" onclick="navigateTo('admin-financial')"><div class="action-icon green"><i class="ti ti-cash"></i></div><h3>Financial</h3><p>Fees, payouts and export</p></div></div></div></div></div>`;
...
PAGES['admin-redemptions'] = `<div class="home-screen"><div class="subpage-header"><div class="subpage-left"><button class="icon-btn" onclick="navigateTo('admin-panel')"><i class="ti ti-arrow-left"></i></button><h2>Redemptions</h2></div><button class="icon-btn" onclick="refreshAdminRedemptions()"><i class="ti ti-refresh"></i></button></div><div class="page-scroll"><div style="display:flex;gap:8px;padding:12px 16px;overflow-x:auto;"><button onclick="filterAdminRedemptions('pending')" id="redemption-filter-pending" style="padding:6px 14px;border-radius:20px;background:#f97316;color:#fff;border:none;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;">Pending</button><button onclick="filterAdminRedemptions('fulfilled')" id="redemption-filter-fulfilled" style="padding:6px 14px;border-radius:20px;background:#fff;color:var(--text-muted);border:1px solid var(--border);font-size:12px;cursor:pointer;white-space:nowrap;">Fulfilled</button><button onclick="filterAdminRedemptions('rejected')" id="redemption-filter-rejected" style="padding:6px 14px;border-radius:20px;background:#fff;color:var(--text-muted);border:1px solid var(--border);font-size:12px;cursor:pointer;white-space:nowrap;">Rejected</button><button onclick="filterAdminRedemptions('all')" id="redemption-filter-all" style="padding:6px 14px;border-radius:20px;background:#fff;color:var(--text-muted);border:1px solid var(--border);font-size:12px;cursor:pointer;white-space:nowrap;">All</button></div><div id="admin-redemptions-list" style="padding:0 16px 16px;"></div></div></div>`;
...
let cachedFinAnalytics = null;
let cachedFinStatements = [];
let cachedFinCampaigns = [];
let cachedFinUsers = [];

async function initAdminFinancial() {
  try {
    const [analytics, statementsData, campaignsData, usersData] = await Promise.all([
      apiFetch('/admin/analytics'),
      apiFetch('/admin/advertisers/financial'),
      apiFetch('/admin/campaigns'),
      apiFetch('/admin/users?role=USER'),
    ]);
    cachedFinAnalytics = analytics;
    cachedFinStatements = statementsData.statements || [];
    cachedFinCampaigns = campaignsData.campaigns || [];
    cachedFinUsers = usersData.users || [];
  } catch (err) {
    console.error('Failed to load financial data:', err.message);
    return;
  }

  var set = function(id, val) { var el = document.getElementById(id); if (el) el.textContent = val; };
  var totalAdminFees = cachedFinStatements.reduce(function(sum, a) { return sum + a.totalAdminFee; }, 0);
  set('fin-total-fees',    window.formatRand(totalAdminFees));
  set('fin-user-wallets',  window.formatRand(cachedFinAnalytics.wallets.totalCashHeld));
  set('fin-data-balances', Number(cachedFinAnalytics.wallets.totalDataHeld).toFixed(0) + ' MB');
  set('fin-adv-spend',     window.formatRand(cachedFinAnalytics.campaigns.totalSpentOnUsers));

  var container = document.getElementById('fin-transactions');
  if (container) {
    if (cachedFinStatements.length === 0) {
      container.innerHTML = "<div style='text-align:center;padding:16px;color:var(--text-muted);font-size:13px;'>No advertisers yet</div>";
    } else {
      container.innerHTML = cachedFinStatements.map(function(a) {
        return "<div style='background:#f9fafb;border-radius:12px;padding:14px;border:1px solid var(--border);margin-bottom:10px;'>" +
          ...
          "<div><p style='font-size:14px;font-weight:700;color:var(--text-primary);margin:0;'>" + (a.company || a.name) + "</p>" +
          "<p style='font-size:11px;color:var(--text-muted);margin:0;'>" + a.email + "</p></div>" +
          "<span style='font-size:12px;font-weight:700;color:#ef4444;'>R " + window.formatAmt(a.totalCharged) + "</span></div>" +
          "<div style='display:flex;gap:8px;font-size:11px;color:var(--text-muted);margin-bottom:10px;'>" +
          "<span>Campaigns: " + a.campaignCount + "</span><span>|</span>" +
          "<span>Admin Fee: R " + window.formatAmt(a.totalAdminFee) + "</span><span>|</span>" +
          "<span>VAT: R " + window.formatAmt(a.totalVat) + "</span></div>" +
          "<button onclick=\"downloadAdvStatement('" + a.id + "')\" style='width:100%;padding:9px;border-radius:20px;background:linear-gradient(135deg,#2d1b8e,#534AB7);color:#fff;font-size:12px;font-weight:700;border:none;cursor:pointer;'>" +
          "<i class=\"ti ti-download\" style=\"margin-right:5px;\"></i>Download Statement</button></div>";
      }).join('');
    }
  }
...
function renderUsersByRegion() {
  ...
  var filtered = allUsers.filter(function(u) {
    ...
    if (searchFilter) {
      ...
      if (name.indexOf(searchFilter) === -1 && email.indexOf(searchFilter) === -1) return false;
    }
    return true;
  });
  ...
  Object.keys(provinces).forEach(function(provKey) {
    ...
    users.forEach(function(u) {
      ...
      var dataBalance = u.wallet ? Number(u.wallet.dataBalance) : 0;
      html += '<div style="border-bottom:1px solid var(--border);padding:10px 12px;display:flex;justify-content:space-between;align-items:center;gap:8px;">';
      ...
      html += '<p style="font-size:11px;color:var(--text-muted);margin:0;">' + (u.email || '') + '</p></div>';
      html += '<div style="display:flex;flex-direction:column;gap:2px;align-items:flex-end;flex-shrink:0;">';
      html += '<span style="font-size:11px;font-weight:700;color:#22c55e;">R ' + window.formatAmt(balance) + '</span>';
      html += '<span style="font-size:11px;color:var(--text-muted);">' + dataBalance.toFixed(0) + ' MB</span>';
      html += '</div></div>';
    });
...
function filterAnalytics(period) {
  ...
  if (!cachedAdminAnalytics) return;
  var d = cachedAdminAnalytics;

  var set = function(id, val) { var el = document.getElementById(id); if (el) el.textContent = val; };
  set('pa-users',       d.users.totalUsers);
  set('pa-tasks',       d.taskCompletions.total);
  set('pa-wallets',     window.formatRand(d.wallets.totalCashHeld));
  set('pa-adv-budgets', window.formatRand(d.campaigns.totalBudget));
  set('pa-advertisers', d.users.totalAdvertisers);
  set('pa-campaigns',   d.campaigns.activeCampaigns);

  drawAnalyticsChart(period, d.registrationsByDay || {});
}

function drawAnalyticsChart(period, registrationsByDay) {
  var canvas = document.getElementById('pa-chart');
  if (!canvas) return;
  var ctx = canvas.getContext('2d');

  var daysToShow = period === 'today' ? 1 : period === 'week' ? 7 : 30;
  var labels = [];
  var data   = [];
  ...
  for (var i = daysToShow - 1; i >= 0; i--) {
    ...
    data.push(registrationsByDay[key] || 0);
  }
...
PAGES['loading'] = `<div style="width:100%;min-height:100vh;background:linear-gradient(160deg,#1a1060 0%,#2d1b8e 50%,#1a1060 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:24px;"><img src="assets/img/logo.png" alt="KwandaData" style="width:80px;height:80px;border-radius:22px;box-shadow:0 10px 28px rgba(0,0,0,0.35);"/><div style="text-align:center;"><h1 style="font-family:sans-serif;font-size:26px;font-weight:700;color:#fff;margin:0 0 6px;">KwandaData&trade;</h1><p style="font-family:sans-serif;font-size:13px;color:rgba(255,255,255,0.7);margin:0;">Connect. Earn. Empower Communities.</p></div><div style="width:140px;height:4px;background:rgba(255,255,255,0.2);border-radius:2px;overflow:hidden;"><div id="loading-bar" style="width:0%;height:100%;background:#fff;border-radius:2px;transition:width 1.2s ease;"></div></div></div>`;
...
function submitDonate() {
  ...
  // Persist sender's deduction to the master users list too, so it
  // carries over next time they log in (login re-reads from kwanda_users)
  if (user) {
    var senderIndex = allUsers.findIndex(function(u) { return u.email === user.email; });
    if (senderIndex !== -1) {
      allUsers[senderIndex].balance = user.balance;
    }
  }
...
// ── Live-update (admin): as users earn, redeem, or advertisers spend
//    budget — possibly in another browser tab — refresh whichever admin
//    screen is currently open so figures stay current without needing
//    to navigate away and back. ──
window.addEventListener('storage', function(e) {
  if (['kwanda_users', 'kwanda_advertisers', 'kwanda_campaigns', '_k_fees', 'kwanda_activity_log'].indexOf(e.key) === -1) return;

  if (document.getElementById('fin-total-fees')) {
    initAdminFinancial();
  }
  if (document.getElementById('pa-users')) {
    filterAnalytics(currentAdminAnalyticsPeriod);
  }
...
document.addEventListener('DOMContentLoaded', async function() {
  // Admin is always exempt from maintenance mode — otherwise nobody could ever turn it back off.
  var advSession = JSON.parse(localStorage.getItem("kwanda_advertiser_session") || "null");
  var isAdmin = advSession && advSession.role === "ADMIN";

  if (!isAdmin) {
    try {
      var data = await apiFetch('/settings');
      if (data.settings && data.settings.maintenanceEnabled) {
        navigateTo('maintenance');
        var msgEl = document.getElementById("maintenance-message");
        if (msgEl) msgEl.textContent = data.settings.maintenanceMessage || "We are currently performing scheduled maintenance.";
        return;
      }
    } catch (err) {
      // If the settings check itself fails (e.g. backend waking up), fail OPEN — never lock
      // real users out of the whole app just because one status check couldn't be reached.
...
var hash = window.location.hash.replace('#', '');
  if (hash.indexOf('reset-password') === 0) {
    ...
    window.pendingResetToken = qIndex !== -1 ? new URLSearchParams(hash.slice(qIndex)).get('token') : null;
...
function exportFinancialSummary() {
  if (!cachedFinAnalytics) { alert('Please open Financial Control first so data can load.'); return; }
  var now = new Date();
  var d   = cachedFinAnalytics;
  ...
  lines.push("----------------------------------------");
  lines.push("Total Users:          " + d.users.totalUsers);
  lines.push("Total Advertisers:    " + d.users.totalAdvertisers);
  lines.push("Total Campaigns:      " + d.campaigns.totalCampaigns);
  lines.push("Active Campaigns:     " + d.campaigns.activeCampaigns);
  lines.push("");

  lines.push("Total User Wallets:   R " + window.formatAmt(d.wallets.totalCashHeld));
  lines.push("Total Data Balances:  " + Number(d.wallets.totalDataHeld).toFixed(0) + " MB");
  lines.push("");
  ...
  var grandAdmin = 0, grandVat = 0, grandCharged = 0, grandBudget = 0;

  if (cachedFinStatements.length === 0) {
    lines.push("No advertisers yet.");
  } else {
    cachedFinStatements.forEach(function(a, i) {
      grandAdmin   += a.totalAdminFee;
      grandVat     += a.totalVat;
      grandCharged += a.totalCharged;

      lines.push("");
      lines.push((i + 1) + ". " + (a.company || a.name));
      lines.push("   Email:            " + a.email);
      lines.push("   Campaigns:        " + a.campaignCount);
      lines.push("   Admin Fees (20%): R " + window.formatAmt(a.totalAdminFee));
      lines.push("   VAT (15%):        R " + window.formatAmt(a.totalVat));
      lines.push("   Total Charged:    R " + window.formatAmt(a.totalCharged));
      lines.push("   Total Spent:      R " + window.formatAmt(a.totalSpent));
      lines.push("   ----------------------------------------");
    });
  }

  cachedFinCampaigns.forEach(function(c) { grandBudget += Number(c.budget); });
...
function exportUsersByRegion() {
  ...
  Object.keys(provinces).forEach(function(provKey) {
    ...
    users.forEach(function(u, i) {
      ...
      lines.push("   Wallet:      R " + window.formatAmt(balance));
      lines.push("   Data:        " + dataBalance.toFixed(0) + " MB");
      lines.push("   Joined:      " + new Date(u.createdAt).toLocaleDateString("en-ZA"));
      lines.push("");
    });
  });
...
function downloadAdvStatement(advId) {
  var statement = cachedFinStatements.find(function(a) { return a.id === advId; });
  if (!statement) { alert('Advertiser not found.'); return; }

  var advCampaigns = cachedFinCampaigns.filter(function(c) { return c.advertiser && c.advertiser.id === advId; });
  var now = new Date();

  var lines = [];
  lines.push("========================================");
  lines.push("   KWANDADATA ADVERTISER STATEMENT      ");
  lines.push("========================================");
  lines.push("Advertiser:   " + (statement.company || statement.name));
  lines.push("Email:        " + statement.email);
  lines.push("Generated:    " + now.toLocaleString("en-ZA"));
  lines.push("========================================");
  lines.push("");

  var runningCampaigns = advCampaigns.filter(function(c) { return c.status === "active"; }).map(function(c) { return c.title; });
  lines.push("Currently Running Campaigns: " + (runningCampaigns.length > 0 ? runningCampaigns.join(", ") : "None"));
  lines.push("");

  if (advCampaigns.length === 0) {
    lines.push("No campaigns yet.");
  } else {
    advCampaigns.forEach(function(c, i) {
      var firstTask = (c.tasks && c.tasks[0]) || {};
      lines.push((i + 1) + ". " + c.title);
      lines.push("   Type:          " + (firstTask.type || 'N/A'));
      lines.push("   Status:        " + c.status);
      lines.push("   Budget:        R " + window.formatAmt(c.budget));
      lines.push("   Admin Fee:     R " + window.formatAmt(c.adminFee));
      lines.push("   VAT (15%):     R " + window.formatAmt(c.vat));
      lines.push("   Total Charged: R " + window.formatAmt(c.totalCharged));
      lines.push("   Spent:         R " + window.formatAmt(c.spent));
      lines.push("");
    });
  }
  ...
  lines.push("TOTALS");
  lines.push("========================================");
  lines.push("Campaigns:        " + statement.campaignCount);
  lines.push("Admin Fees:       R " + window.formatAmt(statement.totalAdminFee));
  lines.push("VAT Collected:    R " + window.formatAmt(statement.totalVat));
  lines.push("Total Charged:    R " + window.formatAmt(statement.totalCharged));
  lines.push("Total Spent:      R " + window.formatAmt(statement.totalSpent));
  lines.push("========================================");
  lines.push("KwandaData™ - Turning Participation into Opportunity");
  lines.push("support@kwandadata.co.za");
  lines.push("========================================");
  ...
  var a       = document.createElement("a");
  a.href      = url;
  a.download  = "Statement_" + (statement.company || advId) + "_" + now.toISOString().slice(0, 7) + ".txt";
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
